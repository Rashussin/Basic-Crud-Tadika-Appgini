	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'admin' => array('Admin', '', 'resources/table_icons/administrator.png', 'Sales'),
			'cikgu' => array('Cikgu', 'Senarai Cikgu Tadika Anakku Gemilang', 'resources/table_icons/client_account_template.png', 'Sales'),
			'pelajar' => array('Pelajar', 'Senarai Pelajar Tadika Anakku Gemilang', 'resources/table_icons/group_edit.png', 'Sales'),
			'Subjek' => array('Subjek', '', 'table.gif', 'Sales')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'admin' => "`admin`.`adminID` as 'adminID', `admin`.`nama` as 'nama', `admin`.`email` as 'email', `admin`.`phone` as 'phone'",
			'cikgu' => "`cikgu`.`cikguID` as 'cikguID', `cikgu`.`nama` as 'nama', `cikgu`.`email` as 'email', `cikgu`.`phone` as 'phone', `cikgu`.`alamat` as 'alamat', IF(    CHAR_LENGTH(`admin1`.`nama`), CONCAT_WS('',   `admin1`.`nama`), '') as 'ditambahOleh', IF(    CHAR_LENGTH(`admin2`.`nama`), CONCAT_WS('',   `admin2`.`nama`), '') as 'adminID'",
			'pelajar' => "`pelajar`.`pelajarID` as 'pelajarID', `pelajar`.`nama` as 'nama', `pelajar`.`umur` as 'umur', `pelajar`.`alamat` as 'alamat', IF(    CHAR_LENGTH(`Subjek1`.`subjekKode`) || CHAR_LENGTH(`Subjek1`.`subjekNama`), CONCAT_WS('',   `Subjek1`.`subjekKode`, ' - ', `Subjek1`.`subjekNama`), '') as 'subjek', `pelajar`.`statusYuran` as 'statusYuran', IF(    CHAR_LENGTH(`admin1`.`nama`), CONCAT_WS('',   `admin1`.`nama`), '') as 'ditambahOleh', IF(    CHAR_LENGTH(`admin2`.`nama`), CONCAT_WS('',   `admin2`.`nama`), '') as 'adminID', IF(    CHAR_LENGTH(`Subjek2`.`subjekNama`), CONCAT_WS('',   `Subjek2`.`subjekNama`), '') as 'subjekID'",
			'Subjek' => "`Subjek`.`subjekID` as 'subjekID', `Subjek`.`subjekKode` as 'subjekKode', `Subjek`.`subjekNama` as 'subjekNama', `Subjek`.`subjekDescription` as 'subjekDescription'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'shippers' => "`shippers` ",
			'admin' => "`admin` ",
			'cikgu' => "`cikgu` LEFT JOIN `admin` as admin1 ON `admin1`.`adminID`=`cikgu`.`ditambahOleh` LEFT JOIN `admin` as admin2 ON `admin2`.`adminID`=`cikgu`.`adminID` ",
			'pelajar' => "`pelajar` LEFT JOIN `Subjek` as Subjek1 ON `Subjek1`.`subjekID`=`pelajar`.`subjek` LEFT JOIN `admin` as admin1 ON `admin1`.`adminID`=`pelajar`.`ditambahOleh` LEFT JOIN `admin` as admin2 ON `admin2`.`adminID`=`pelajar`.`adminID` LEFT JOIN `Subjek` as Subjek2 ON `Subjek2`.`subjekID`=`pelajar`.`subjekID` ",
			'Subjek' => "`Subjek` "
		);
